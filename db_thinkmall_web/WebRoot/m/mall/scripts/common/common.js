...
//	    	alert("进入初始化方法"+servticket_id);
	    ...
	    //错误号
	    var errorcode = "";
	    var error_cwh = appUtils.getSStorageInfo("errorcode");
	    if(error_cwh){
	    	errorcode = error_cwh;
	    }else{
	    	errorcode = getUrlParam('errorcode');
	    	if(errorcode){
	    		appUtils.setSStorageInfo("errorcode","errorcode");
	    	}
	    }
		...
		//商城token
		var temp_token_mall = "";
		var token = appUtils.getSStorageInfo("temp_token_mall");
		if(token){
			temp_token_mall = token;
		}else{
			temp_token_mall = getUrlParam('temp_token_mall');
			if(temp_token_mall){
//				alert("临时token"+temp_token_mall);
				appUtils.setSStorageInfo("temp_token_mall",temp_token_mall);
			}
		}
		
		var client_id = "";
		var clientid = appUtils.getSStorageInfo("client_id");
		if(clientid){
			client_id = clientid;
		}else{
			client_id = getUrlParam('client_id');
			if(client_id){
//				alert("client_id的值为："+client_id);
				appUtils.setSStorageInfo("client_id",client_id);
			}
		}
		
		//是否从中焯到注册
		var zjregister = "";
		var register = appUtils.getSStorageInfo("zjregister");
		if(register){
			zjregister = register;
		}else{
			zjregister = getUrlParam('zjregister');
			if(zjregister){
//				alert("zjregister的值为："+zjregister);
				appUtils.setSStorageInfo("zjregister",zjregister);
			}
		} 
		
		//是否从中焯到忘记密码
		var zjforgetpassword = "";
		var forgetpassword = appUtils.getSStorageInfo("zjforgetpassword");
		if(forgetpassword){
			zjforgetpassword = forgetpassword;
		}else{
			zjforgetpassword = getUrlParam('zjforgetpassword');
			if(zjforgetpassword){
//				alert("zjforgetpassword的值为："+zjforgetpassword);
				appUtils.setSStorageInfo("zjforgetpassword",zjforgetpassword);
			}
		} 
		
		var clientlogin = "";
		var client_login = appUtils.getSStorageInfo("clientlogin");
		if(client_login){
			clientlogin = client_login;
		}else{
			clientlogin = getUrlParam('clientlogin');
			if(clientlogin){
				appUtils.setSStorageInfo("clientlogin",clientlogin);
			}
		}
		
		//是否从banner进入
		var isFrom_Ba = "";
		var isFromBa = appUtils.getSStorageInfo("isFromBa");
		if(isFrom_Ba){
			isFromBa = isFrom_Ba;
		}else{
			isFromBa = getUrlParam('isFromBa');
			if(isFromBa){
				appUtils.setSStorageInfo("isFromBa",isFromBa);
			}
		}
		
		if (clientlogin && clientlogin == "1" && isFromBa && isFromBa == "1") {
			//获取用户登录信息
...
//					alert("走回调："+user_id);
	...
	function filterLoginOut(){
		...
		var temp_token_mall = appUtils.getSStorageInfo("temp_token_mall");
		var client_id = appUtils.getSStorageInfo("client_id");
		if(clientlogin && clientlogin == "1" && temp_token_mall && client_id){
//			alert("common.js中的clientlogin="+clientlogin);
			getLoginTrade();
		}else if(clientlogin && clientlogin == "1"&& servticket_id && !errorcode){
			getLoginClientInfo();
		}else{
	...
	function checkPermission(checkInParam){
		...
		if((checkLoginPage && checkLoginPage.indexOf(pageCode) > -1) || (notCheckLoginPage && notCheckLoginPage.indexOf(pageCode) < 0) ){
			...
			if(!user_id){
				...
				if (clientlogin && clientlogin == 1 && !errorcode && !servticket_id || clientlogin && clientlogin == "1" && !temp_token_mall && !client_id) {
					getLoginClientInfo(true);
				}else if(clientlogin && clientlogin == 1 && errorcode && !servticket_id){
...
//			    	pwdParam.push(pwd);
	...
	function checkUserIsLogin(isCheckFundAccount, isPageBack, loginInPageCode, loginInPageParam, isBuy){
		...
		if(userInfo == null || userInfo == ""){
			// 产品购买页面 提示用户使用资金账号登录
			if(clientlogin && clientlogin == "1" && !errorcode && !servticket_id || clientlogin && clientlogin == "1" && !temp_token_mall){
//				if (isBuy) {
