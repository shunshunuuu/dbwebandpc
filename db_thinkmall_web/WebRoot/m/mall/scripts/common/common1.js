...
define(function(require, exports, module){ 
	...
	function firstLoadFunc(){
		...
		/**********   中焯app统一start   *********/
		// 判断链接来源是否来自中焯APP
		var isFromApp = getUrlParam('src_type');
		if (isFromApp) {
			appUtils.setSStorageInfo("isFromApp",isFromApp);
		}
		//票据id
		var servticket_id = getUrlParam('servticket_id');  
		if(servticket_id){
			appUtils.setSStorageInfo("servticket_id",servticket_id);
		}
		//资金账号
		var fundAccount = getUrlParam('fundAccount');
		if(fundAccount){
			appUtils.setSStorageInfo("fundAccount",fundAccount);
		}
		
		// 获取返回用户信息，如果为空表示未登录
		var user_token = getUrlParam('user_token');
		if (user_token) {
			appUtils.setSStorageInfo("user_token",user_token);
		}
		// 获取返回手机号码
		var mobilecode = getUrlParam('mobilecode');
		if (mobilecode) {
			appUtils.setSStorageInfo("mobilecode",mobilecode);
		}
	...
	function getLoginClientInfo(flag){
		...
		var user_token = appUtils.getSStorageInfo("user_token");
		var mobilecode = appUtils.getSStorageInfo("mobilecode");
		//alert(user_token + mobilecode);
		if(user_token && user_token != "null" &&  mobilecode && mobilecode !="null"){
			var param = {
					"user_token" : user_token,
					"mobilecode" : mobilecode
			};
			service.getClientInfoFromZZ(param,function(resultVo){
				
				var error_no = resultVo.error_no;
				var obj = null;
				var error_info = resultVo.error_info;
				if(error_no == "0"){
					obj = JSON.parse(resultVo.results[0].result);
					pageGlobal.fund_account = obj.FUNDACCOUNT;//从中焯获得的资金账号
					var tradepwd_zz = obj.PASSWORD;  //交易密码
					
					//对中焯传递的交易密码进行解密,进行模拟登录
					if (tradepwd_zz) {
						decodePwdFromZZ(tradepwd_zz);
					}
				}
			});
		}else{
	 ...
	 * 对中焯的交易密码进行处理
	 */
	function decodePwdFromZZ(tradepwd_zz){
		
		var param = {
				"password" : tradepwd_zz
		};
		
		service.decodePwaFromZZ(param,function(resultVo){
			var error_no = resultVo.error_no;
			var error_info = resultVo.error_info;
			if (error_no == "0"){
				var trade_pwd_mw = resultVo.results[0].trade_pwd;
				
				//对交易密码进行思迪的加密
				if(trade_pwd_mw){
					rsaEncryptTradePwd(trade_pwd_mw);
				}else{
    ...
    /**
	 * 处理模拟登陆失败(此处的处理待商城模拟登录的功能号提供了再进行实现)
	 */
...
//			 str = "http://action:10090/?loginType=1&&loginKind=0&&jsfuncname=common.firstLoadFunc()&&url="+global.url_loginIn_zzapp+"/"+curPageCode+".html?src_type=zzapp&product_id="+productId+"&user_token=%s&mobileCode=%s"; 
			 str = "http://action:10090/?logintype=1&loginkind=1&url="+global.url_loginIn_zzapp+"/"+curPageCode+".html?product_id="+productId;
		 }else{
//			  str = "http://action:10090/?loginType=1&&loginKind=0&&jsfuncname=common.firstLoadFunc()&&url="+global.url_loginIn_zzapp+"/"+curPageCode+".html?src_type=zzapp&user_token=%s&mobileCode=%s"; 
	...
	function checkPermission(checkInParam){
		...
		// 判断链接来源是否来自中焯APP,如果是，则模拟登录
		if (isFromApp && isFromApp == "zzapp" && pageCode && pageCode == "login/userLogin") {
			var flag = (/^\d+$/.test(productId)) ? productId : true;
			getLoginClientInfo(flag);
			//getLoginClientInfo(true);
		}else{
			return true;
		}
