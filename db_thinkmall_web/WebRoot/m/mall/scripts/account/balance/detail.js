				...
				if (results.length > 0) {
					...
					if (result.account_status == "0") {
						...
						if (product_code &&　product_code　== 'AF0008') {
							layerUtils.iAlert("您已开通现金宝，不能再开通东方基金余额宝");
						}
						...
						$(_pageId + "#userableSell_").html(parseFloat(pageGlobal.userableSell).toFixed(2)); // 可用份额
 						...
 						if (type && type == '1') {
 							$(_pageId + ".flex-father a").removeClass("on");
 							$(_pageId + ".flex-father").find("a").eq(1).addClass("on");
...
//				"fina_belongs" : "3",
				...
				if(error_no == "0"){
					...
					if (results.length > 0) {
						...
						if (is_risk == "1" || is_risk == "") { // 开通成功， 跳转到成功页面
							layerUtils.iMsg(-1, "抱米余额理财开通成功，请设置留存金额");
	...
	function cashTaking(tot_price){
		...
		service.cashTaking(param, function(data){
			...
			if(error_no == "0"){
				var results = data.results[0];
				layerUtils.iMsg(-1, "赎回成功");
				queryCashButler(1);
				$(_pageId + "#redeemBox").hide();
				$(_pageId + "#redeemPrice").val("");
	...
	function bindPageEvent(){
		...
		appUtils.bindEvent(_pageId + "#openBtn", function(){
			...
			if(userInfo){
				...
				if (client_rights) {
					...
					if(branch_no == constants.balance_product.branch_no){
						layerUtils.iAlert("尊敬的投资者：</br>我公司长沙芙蓉中路证券营业部自2016年12月14日起至2017年6月13日止暂停代销金融产品业务，敬请周知。</br>如有疑问，请咨询客服电话95360。</br>东北证券股份有限公司");
						return false;
					}
					// 判断是否开通e权限
		...
		appUtils.bindEvent(_pageId + "#submitSetBtn", function(e){
			...
			if (parseFloat(totPrice.val()) < 10 ) {
				totPrice.val("10");
				layerUtils.iMsg(-1, "您输入的保留金额小于10元，系统将按默认10元作为您的保留金额");
				return;
			}
			var param = {
				"type" : "2", // 预留金额修改
			...
			service.productRegister(param, function(data){
				...
				if(error_no == "0"){
					...
					if (error_info.indexOf("[150270][基金账户为异常状态]") != -1) {
						layerUtils.iMsg(-1, "请在T+2日产品开通成功后，再设置保留金额！");
					}else{
						layerUtils.iMsg(-1, error_info);
					}
					return false;
		...
		appUtils.bindEvent(_pageId + "#sub_btn_out", function(){
			layerUtils.iConfirm("您确定要取消参与抱米余额理财吗?", function(){
				var param = {
					"type" : "1", // 取消产品登记
					"fund_account" : pageGlobal.fundAccount,
					"product_code" : global.product_code_balance 	
