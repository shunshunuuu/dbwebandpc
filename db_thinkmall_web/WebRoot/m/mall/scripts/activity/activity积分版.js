...
define(function(require,exports,module){
	...
	function isLogin(){
		...
		// 推荐码为null
		/*if(recomCode.length <= 0 && recomCode == null ){ 
		    	appUtils.pageBack();
		    	return false;
		}*/
	...
	function queryActivityInfo(){
		...
		var param = {
				...
				//"user_id" : common.getUserId()    // 获取用户ID 
				
		};
		
		/*回调函数*/
		var callBack = function(resultVo){
			//console.log(JSON.stringify(resultVo));//打印后台返回参数到控制台
	 ...
	 */
	 function queryLuckyResults(){
		    ...
		    service.queryLuckyResults(param,callBack,{"isShowWait":false});
	 }
	...
	var lottery = {
		...
		_destroy: function () {
			...
			_this.options.speed = _this.options.initSpeed;
			_this.options.isRunning = false;
			clearTimeout(_this.getResultTimer);
			_this._disable();
		},
		// 转盘滚动
		_roll: function () {
			var _this = this;
			_this.container.find('[lottery-unit-index=' + _this._index() + ']').removeClass("jggactive");
			++_this.options.index;
			_this.container.find('[lottery-unit-index=' + _this._index() + '].lottery-unit').addClass("jggactive");
