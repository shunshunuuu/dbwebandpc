...
//				var purchase_rates = result.purchase_rates;// 申购费率
				...
				if(fund_type == "2"){
					var profit_of_10_thousands = result.profit_of_10_thousands;
					$(_pageId + "#nrmc").html("万份收益");
					$(_pageId + "#current_price").html(profit_of_10_thousands);
				}else{
					$(_pageId + "#nrmc").html("最新净值");
...
//				$(_pageId + "#share_type").html(share_type).parent().parent().show();
				...
				if(fund_type == "2"){
					var ahtml = '<em>'+annual_profit+'</em><i>%</i>近七日年化收益';
			...
			if(error_no == "0"){
					...
					if (results && results.length > 0) {
						...
						for (var i = 0; i < resultsLen; i++) {
							...
							if(seven_days_annual_profit == ""){
								seven_days_annual_profit = 1.00;
							}
							
							// 更新最大净值
							if (seven_days_annual_profit > maxPrice) {
								maxPrice = seven_days_annual_profit;
							}
							
							// 更新最小净值
							if (i == 0 || seven_days_annual_profit < minPrice) {
								minPrice = seven_days_annual_profit;
							}
							...
							xDates.push(navDate);
							dataNav.push(parseFloat(seven_days_annual_profit).toFixed(3)-0);
							datas.push(dataNav);
...
//						$(_pageId +"#tab_price").find("div").eq(1).find("a").html("万份收益");
						...
						for (var i = 0; i < resultsLen; i++) {
							var item = results[i];
							var profit_per_million = item.profit_per_million;//万份收益
							if(profit_per_million == ""){
								profit_per_million = 1.00;
							}
							
							// 更新最大净值
							if (profit_per_million > maxPrice) {
								maxPrice = profit_per_million;
							}
							
							// 更新最小净值
							if (i == 0 || profit_per_million < minPrice) {
								minPrice = profit_per_million;
							}
							...
							navDate = navDate.substring(5, navDate.length);
							xDates.push(navDate);
							dataNav.push(parseFloat(profit_per_million).toFixed(3)-0);
							datas.push(dataNav);
	...
	function productPrice(num) {
		...
		service.productPrice(param, function(data){
			...
			if(error_no == "0"){
					...
					$('#container').empty(); 
					if (results && results.length > 0) {
						$(_pageId +"#tab_price").find("div").eq(0).find("a").html("净值走势");
						$(_pageId +"#tab_price").find("div").eq(1).find("a").html("历史净值");
	...
	// 净值走势图
	function graphPicse(options){
		
		$(_pageId + "#nav").css("width", $(window).width() - 20).css("height", 160);
		var min = parseFloat(options.min);
		var max = parseFloat(options.max);
		var yTickInterval = (max - min) / 3; //y轴刻度大小
		yTickInterval = Number(yTickInterval.toFixed(3));
		...
		var startMin = parseFloat(min); // y轴起始值
		var xTickInterval = parseInt((options.xDates.length - 1) / 3); // x轴刻度间隔
		var containerWidth = $(_pageId + "#nav").width() - 100; // 用户计算toolTip不超出边界
		var tipDates = options.tipDates; // 提示框中展现的日期数组
		$('#nav').highcharts({
			chart : {
				backgroundColor : "#fff",
...
//			}
		...
		// 七日年化点击时间刻度月、季、半年、年事件
		appUtils.bindEvent(_pageId + "#tab_price .row-11", function(){
			if ($(this).hasClass("act")) {
				return;
			}
			
			var rowList = $(_pageId + "#tab_price .row-11").removeClass("act"); // 移除所有row的act样式
			var index = rowList.index(this);
			$(this).addClass("act"); // 激活当前点击对象的样式
		...
		// 万份收益点击时间刻度月、季、半年、年事件
		appUtils.bindEvent(_pageId + "#tab_price .row-111", function(){
			if ($(this).hasClass("act")) {
				return;
			}
			
			var rowList = $(_pageId + "#tab_price .row-111").removeClass("act"); // 移除所有row的act样式
			var index = rowList.index(this);
			$(this).addClass("act"); // 激活当前点击对象的样式
