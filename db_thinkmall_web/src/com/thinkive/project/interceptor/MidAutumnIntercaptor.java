...
public class MidAutumnIntercaptor implements Interceptor
{
    Logger logger = Logger.getLogger(MidAutumnIntercaptor.class);

	@Override
	public Result intercept(ActionInvocation invocation) throws Exception {
		
		logger.info("进入MidAutumnIntercaptor中秋活动参数拦截器");
		...
		if(param != null){
			...
			if(funcNo.equals("1105092")){
				...
				logger.info("phone_reco:"+phone_reco);
				...
				logger.info("Url地址："+FuncConstant.MIDAUTUMN_URL);
				result = paramsSend(FuncConstant.MIDAUTUMN_URL,paramsMap);
				return result;
			}
			
		}else{
			
			logger.error("接入层请求MidAutumnIntercaptor的参数为空！");
			return result;
		}
	...
	private String md5Url(Map<String, String> paramsMap)
	{
		...
		String token_key = FuncConstant.MIDAUTUMN_TOKEN_KEY;
	...
	public static void main(String[] args) {
		...
		MidAutumnIntercaptor it = new MidAutumnIntercaptor();
		String url = FuncConstant.MIDAUTUMN_URL;
		Map<String,String> paramsMap = new HashMap<String, String>();
		paramsMap.put("phone", "18229398968");
		paramsMap.put("phone_reco", "17004964322");
		paramsMap.put("type","5");
		paramsMap.put("token_key", FuncConstant.MIDAUTUMN_TOKEN_KEY);
