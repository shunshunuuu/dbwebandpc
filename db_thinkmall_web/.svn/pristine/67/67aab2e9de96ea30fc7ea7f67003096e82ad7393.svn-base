...
define(function(require, exports, module){ 
	var _pageId = "#finan_fixedInvestment_placeSuccess "; // 当前页面ID
	 ...
	 */
	function init(){
		var product_name = appUtils.getPageParam("product_name"); // 产品名称
		var product_code = appUtils.getPageParam("product_code"); // 产品名称
		var trade_money = appUtils.getPageParam("trade_money"); // 定投金额
		var trade_date = appUtils.getPageParam("trade_date"); // 交易日期
		var first_trade_y_m_mall = appUtils.getPageParam("first_trade_y_m_mall"); // 开始时间
		var expiry_date = appUtils.getPageParam("expiry_date"); // 结束时间
		
		$(_pageId + "#productName").html(product_name + '('+ product_code +')');
		$(_pageId + "#tradeMoney").html(trade_money + '元');
		$(_pageId + "#tradeDate").html( '每月/'+ trade_date +'号');
		$(_pageId + "#first_trade_y_m_mall").html(first_trade_y_m_mall);
		$(_pageId + "#expiryDate").html(expiry_date);
	}
...
//				var purchase_rates = result.purchase_rates;// 申购费率
	...
	function bindPageEvent(){
		...
		appUtils.bindEvent(_pageId + ".back_btn", function(){
			appUtils.pageInit("finan/fixedInvestment/placeSuccess", "main", {});
		});
		
		// 定投管理
		appUtils.bindEvent(_pageId + "#manage", function(){
			appUtils.pageInit("finan/fixedInvestment/placeSuccess", "account/myOrder", {"_pageCode":"place"});
		});
		
		// 再逛逛
		appUtils.bindEvent(_pageId + "#go", function(){
			appUtils.pageInit("finan/fixedInvestment/placesuccess", "main", {});
		});
	}
