...
/*							try
			...
			// 判断需要过滤入参
			String filterParam = Configuration.getString("system.filterParam");
			if(StringHelper.isNotBlank(filterParam)){

				String[] fParams = StringHelper.split(filterParam, "|");
				if(fParams != null && fParams.length > 0)
				{
					for(int i = 0; i < fParams.length; i ++)
					{
						String fParam = fParams[i];
						String field = MapHelper.getString(param, fParam);
						
						String filterCharacter = Configuration.getString("system.filterCharacter");
						if(StringHelper.isNotBlank(filterCharacter)){
							
							String[] fCharacters = StringHelper.split(filterCharacter, "|");
							if(fCharacters != null && fCharacters.length > 0)
							{
								for(int j = 0; j < fCharacters.length; j ++)
								{
									String fCharacter = fCharacters[j];
									String value = MapHelper.getString(param, fCharacter);
									
									if (field.indexOf(value) > -1) {
										field.replaceAll(value,""); // 将所有存在的特殊字符的字段中的特殊字符替换成空
									}
								}
							}
