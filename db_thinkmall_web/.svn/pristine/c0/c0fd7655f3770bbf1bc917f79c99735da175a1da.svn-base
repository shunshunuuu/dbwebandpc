		...
		(function($){
			...
			Scroll:function(opt,callback){
			        ...
			        var lineH=_this.find("li:first").height(), //获取行高
			            line=opt.line?parseInt(opt.line,6):parseInt(this.height()/lineH,10), //滚动的行数，默认为一屏，即父容器高度
			            speed=opt.speed?parseInt(opt.speed,6):5000, //卷动速度，数值越大，速度越慢（毫秒）
			            timer=opt.timer?parseInt(opt.timer,6):5000; //滚动的时间间隔（毫秒）
			        if(line==0) line=1;
			...
			$(document).ready(function(){
		     	$("#scroll_vertical").Scroll({line:1,speed:10000,timer:30000});
			});
	}
