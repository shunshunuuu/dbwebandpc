...
//	var	platform = gconfig.platform; 
	...
	function init(){
		...
		if(pageGlobal.productCode == constants.product_code.new_client_product){
			...
			if(global.kh_data_star && global.kh_data_end){
				if (!(open_date && global.kh_data_star <= open_date && open_date <= global.kh_data_end)) {
					layerUtils.iAlert("亲，真遗憾，你不符合此次新客专享活动条件！","",function(){
						appUtils.pageInit("finan/buy", "finan/finanDetail", {"product_id":pageGlobal.product_id});
				...
				service.newClientOrderNum(param, function(data){
					...
					if (error_no == "0") {
						...
						var is_check_buy = result.is_check_buy;//产品是否已购完
						if(is_check_buy == "0"){
							layerUtils.iAlert("亲，本期宝贝已卖光啦~","",function(){
								appUtils.pageInit("finan/buy", "finan/market", {});
							},"更多精彩，尽在商城理财");
							return false;
						}
					} else {
		...
		if(client_rights.indexOf("e") == -1){
			layerUtils.iAlert("您尚未签署电子签名约定书，请先至我司官网网上营业厅或营业部柜台签署！","",function(){
				pageGlobal.flag = false;
				var sub_type = appUtils.getPageParam("sub_type");
				var product_id = appUtils.getPageParam("product_id");
				isBuy(pageGlobal.product_id,sub_type); // 是否购买过
				localIsBuy(pageGlobal.product_id); // 查看本地是否购买过
				appUtils.pageBack();
			},"同意");
		}else{
			var sub_type = appUtils.getPageParam("sub_type");
		...
		userAsset(); // 用户资产
		queryContract(pageGlobal.product_id); // 查询协议
		userBank(); // 查询用户持有的银行卡
	...
	function initView(){
		...
		if(prePageCode == "finan/finanAgreementOpen"){
			var buy_Value = appUtils.getPageParam("buy_Value");
			$(_pageId + "#buyValue").val(buy_Value);
		}
	 ...
	 * 本地查看是否购买过
	 */
	function localIsBuy(productId){
		var param = {
				"user_id" : common.getUserId(),
				"product_id" : productId
			};
			service.localIsBuy(param, function(data){
				var error_no = data.error_no;
				var error_info = data.error_info;
				if(error_no == "0"){
					var results = data.results;
					pageGlobal.local_is_bought = results[0].is_bought;// 0 未买过  1 买过
				...
				if (results.length > 0) {
					...
					for (var i  =0; i < results.length; i++){
						if(results[i].agreement_title != null && results[i].agreement_title != ""){
							allProtocolStr += '<a style="font-size: .12rem;" href="' + results[i].url + '" data-id="'+results[i].agreement_id+'" data-title="'+results[i].agreement_title+'" data-url="'+results[i].url+'">《'+results[i].agreement_title+'》</a>';
						}
					}
					$(_pageId+" #contact").html(allProtocolStr);
					$(_pageId+" #amount").attr("style","font-size: .12rem;");
					$(_pageId+" .text-right").attr("style","font-size: .12rem;");
					$(_pageId+" #bankCardCss").attr("style","font-size: .12rem;");
					$(_pageId+" #bondCss").attr("style","font-size: .12rem;");
	...
	function finanInfo(productId){
		...
		service.finanInfo(param,function(data){
			...
			if(error_no == "0"){
				...
				// 银行理财产品fina_belongs = 2
				if(pageGlobal.fina_belongs && pageGlobal.fina_belongs == "2"){
					pageGlobal.isOrder = "1"; // 直接生产订单
				}
			...
			service.openCashButler(param,function(data){
				...
				if(result.length > 0){
					...
					for (var i = 0; i < result.length; i++) {
						...
						if(pageGlobal.sub_type && pageGlobal.sub_type == "0"){
						...
						}else if(pageGlobal.sub_type == "1" && pageGlobal.fina_belongs == "2"){ // 银行理财产品
							if(item.trans_account && item.trans_account != ""){
								// 开通了直接购买
								productBuy(false);
							}else{
								layerUtils.iAlert("首次购买银行理财产品需前往营业部柜台签约</br>详情可拨打95360咨询！");
								return false;
							}
						}else if(pageGlobal.sub_type == "1" && pageGlobal.fina_belongs == "5"){
							var userInfo = appUtils.getSStorageInfo("userInfo"); // 用户信息
							var client_rights = JSON.parse(userInfo).client_rights; // 获取资金账号
							if(client_rights.indexOf("e") == -1){
							...
							if(item.trans_account && item.trans_account != ""){
								...
								if (prePageCode && prePageCode == "finan/finanAgreementOpen") {
									// 开通了直接购买
									productBuy(false);
								}else{
									...
									var param = {
										"product_id" : pageGlobal.product_id,	
										"product_code" : pageGlobal.productCode,	
										"sub_type" : pageGlobal.sub_type,	
										"buy_Value" : buy_Value
									};
									appUtils.pageInit("finan/buy", "finan/finanAgreementOpen", param); // 开通证劵理财户协议
									return false;
								}
							}
						}else if(pageGlobal.sub_type == "1"){
							if(item.is_open && item.is_open == "1"){
								// 开通了基金
								if (item.fund_company == pageGlobal.register_corp_code) {
									// 开通了直接购买
...
*/			} else if (index == 1) {
		...
		appUtils.bindEvent($(_pageId + ".sub_btn"), function(){
			// 新客专享的收益凭证类理财产品
			if(pageGlobal.productCode == constants.product_code.new_client_product){
				if(pageGlobal.is_bought == "0" && pageGlobal.local_is_bought == "0"){// 0 未买过  1 买过
					var buyValue = $(_pageId + "#buyValue").val();
					if(parseFloat(buyValue) > "200000" ){
						layerUtils.iMsg(-1, "此产品新客专享购买上限金额为20万！");
						return false;
					}
				}else{
					layerUtils.iMsg(-1, "你已经购买过，此产品只允许客户购买一次！");
					return false;
				}
			}
			if (pageGlobal.flag) {
