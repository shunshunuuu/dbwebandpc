...
define(function(require, exports, module){ 
	var _pageId = "#account_userCenter1 "; // 当前页面ID
...
//		}
	...
	function bindPageEvent(){
				...
				service.loginOut({}, function(data){
	    			...
	    			if (error_no == "0"){
	    				...
	    				if(isFromApp && isFromApp == "zzapp"){
	    					...
	    					appUtils.pageInit("account/userCenter1", "main", {});
	    				}
	    			} else {
	    				layerUtils.iMsg(-1,error_info);
	    				return false;
		...
		appUtils.bindEvent(_pageId + "#login_nav_list a", function(){
			...
			case 3: 
				// 我的现金管家
				appUtils.pageInit("account/userCenter1", "account/cashbutler/detail", {"backPage" : "account/userCenter1"});
				break;
			case 4: // 我的理财
				if (common.checkUserIsLogin(true, false)) {
					appUtils.pageInit("account/userCenter1", "account/myFinancial", {});
				}
				break;
			case 5: // 实时货币基金
				appUtils.pageInit("account/userCenter1", "account/realOrder", {});
				break;
			case 6: // 我的订单
				if (common.checkUserIsLogin(true, false)) {
					appUtils.pageInit("account/userCenter1", "account/myOrder", {});
				}
				break;
			case 7: // 我的东北币
				appUtils.pageInit("account/userCenter1", "account/myIntegral", {});
				break;
			case 8: // 活动订单
				appUtils.pageInit("account/userCenter1", "active/myOrder", {});
				break;
			case 9: // 银证转账
				if (common.checkUserIsLogin(true, false)) {
					appUtils.pageInit("account/userCenter1", "account/stockBank", {});
				}
				break;
			case 10: // 我的资料
				appUtils.pageInit("account/userCenter1", "account/myProfile", {});
				break;
			case 11: // 风险测评
				if (common.checkUserIsLogin(true, false)) {
					queryRiskLevel();
				...
				break;
			case 12: // 我的银行卡
				appUtils.pageInit("account/userCenter1", "account/myBankCard", {});
				break;
			case 13: // 我的账单
				if (common.checkUserIsLogin(true, false)) {
					var userInfo = appUtils.getSStorageInfo("userInfo"); // 用户信息
				...
				break;
			case 0: // 我要推广
				appUtils.pageInit("account/userCenter1", "account/promote", {});
				break;
			case 1: // 推荐查询
				appUtils.pageInit("account/userCenter1", "account/promoteQuery", {});
				break;
			case 2: // 中奖查询
				appUtils.pageInit("account/userCenter1", "activity/winList", {});
				break;
			default:
				break;
			}
		...
		service.queryRiskLevel(reqParam, function(data){
			...
			if(error_no == "0"){
				...
				if (results.length > 0) {
					...
					pageParam.backPage = "account/userCenter1";
					appUtils.pageInit("account/userCenter1", "register/riskAssDetail", pageParam);
				} else {
					appUtils.pageInit("account/userCenter1", "register/riskAssessment", {"backPage" : "account/userCenter1"});
				}
			}else{
				layerUtils.iMsg(-1,error_info);
				return false;
