...
define(function(require, exports, module){ 
	var _pageId = "#finan_search "; //当前页面ID
	...
	function initSearchHistory(){
		...
		var historyEle = $(_pageId + ".search_page .history_list").empty();
		if (searchHistory) {
			searchHistory = JSON.parse(searchHistory);
			for (var i = searchHistory.length - 1; i >= 0; i--) {
				historyEle.append('<span>' + searchHistory[i] + '</span>'); 
			}
			
			appUtils.bindEvent(_pageId + ".search_page .search_history span", function(){
	...
	function searchProduct(searchValue, searchType){
		var param = {
			...
			"pageSrc" : "finan/search",
			"searchType" : searchType
		};
		
		appUtils.pageInit("finan/search", "finan/market", param);
	}
	...
	function searchHistory(){
		var searchValue = $(_pageId + "#search_value").val();
		var searchHistory = appUtils.getLStorageInfo("searchHistory");
		if (!searchHistory) {
			searchHistory = [];
		} else {
	...
	function bindPageEvent(){
		appUtils.bindEvent(_pageId + ".back_btn", function(){
			appUtils.pageInit("finan/search", "finan/market", {});
		});
		...
		// 点击基金类型查询
		appUtils.bindEvent(_pageId + ".search_page .label_box span", function(){
			var searchValue = $(this).attr("val");
			searchProduct(searchValue, "0");
		});
